 ...
 * @LastEditors: Hinsane
 * @LastEditTime: 2022-06-06 12:35:29
-->
<template>
  <div>
    <div id="order-Statistics-header">
      <a-layout>
        <a-layout-header :style="{ background: '#fff' }" class="search-header">
          <span style="font-size: 20px; margin: 0 20px">新增订单统计</span>
    ...
    exportExcel() {
      if (this.search.orderStatistics.type === "") {
        ...
        this.exportTimer = setTimeout(async() => {
          //访问后台导出文件...
          await this.getAllExportList({'type':this.search.orderStatistics.type,'tableName':'orders'})
